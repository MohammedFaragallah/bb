...
const hasDeclarativeSideEffectsSupport = (
	...
	if (callTimeOptions?.withDeclarativeSideEffectsSupport) return true;
	if (options?.withDeclarativeSideEffectsSupport) return true;
	return false;
};
